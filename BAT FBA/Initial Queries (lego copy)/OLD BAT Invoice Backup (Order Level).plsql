Q_SHIP
------------------
select distinct invs.shipment_Gid SHIPMENT_ID,invs.invoice_gid INVOICE_ID , om.order_release_gid ORDER_ID from invoice_shipment invs,invoice_status invst,order_movement om,glogowner.shipment_involved_party shinv, glogowner.contact con
where invs.invoice_gid=invst.invoice_gid 
...
shipment s
where 
exists(select 1 from ALLOCATION_BASE ab where ab.alloc_type_qual_gid='PLANNING' and  ab.alloc_seq_no=aord.alloc_seq_no )and 
...
,s.T_ACTUAL_COST_CURRENCY_GID Customer_Recharge_Currency

/*,null Customer_Recharge_Value	 */
...
(select glog_util.remove_domain(VAT_CODE_GID) from vat_analysis VA, invoice_shipment invs where VA.invoice_gid=invs.invoice_gid and invs.invoice_gid=:INVOICE_ID) TREATMENT_CODE,
