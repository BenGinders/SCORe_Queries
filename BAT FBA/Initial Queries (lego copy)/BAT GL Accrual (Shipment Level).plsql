Q_SHIP
----------------------
select distinct shipment_gid SHIPMENT_ID from (select   ssh.shipment_gid,shinv.involved_party_contact_gid
from Ie_Shipmentstatus ies,ss_status_history ssh,shipment_involved_party shinv, contact con,shipment s
where 
shinv.shipment_gid = s.shipment_gid 
...
and ssh.shipment_gid like 'LEGO.%' and
...
and not exists 
(select 1 from invoice_shipment inv_s, invoice_status invst where inv_s.shipment_gid = ssh.shipment_gid and inv_s.shipment_gid like 'LEGO.%' and inv_s.invoice_gid=invst.invoice_gid and invst.status_value_gid in('LEGO.APPROVAL_APPROVED_AUTO','LEGO.APPROVAL_APPROVED_MANUAL')) 
group by  ssh.shipment_gid,shinv.involved_party_contact_gid)
...
shipment s where 
s.shipment_gid like 'LEGO.%' and inv.ATTRIBUTE2 (+) not in('ARCHIVED','ARCHIVED_RPA_ERROR') and 
inv.invoice_gid(+)=invs.invoice_gid and 
invs.shipment_gid(+)=s.shipment_gid and 
...
invoice inv,
invoice_remark ir,
shipment s where s.shipment_gid like 'LEGO.%'  
AND  (:P_FBA_OTM_RESPONSIBILITY in (select  SR.shipment_refnum_value  from shipment_refnum  sr  where 
SR.shipment_refnum_qual_gid='LEGO.LLP_RESPONSIBILITY' and sr.shipment_gid = s.shipment_gid) 
or :P_FBA_OTM_RESPONSIBILITY IS NULL) 
AND inv.invoice_gid=ir.invoice_gid and 
ir.remark_text=s.shipment_gid and inv.attribute2 not in ('ARCHIVED','ARCHIVED_RPA_ERROR')
