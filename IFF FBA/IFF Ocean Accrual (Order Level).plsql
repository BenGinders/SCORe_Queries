--! 06.IFF Ocean Accrual (Order Level) 
-- > Notes
...
FILTER_CTE AS 
 ( 
  ...
  WHERE 
    ...
    AND RD(S.TRANSPORT_MODE_GID) IN ('OCEAN-FCL','OCEAN-LCL','OCEAN','VESSEL-CO','OCEAN-FCL-ROAD','OCEAN-LCL-ROAD')
    AND S.SHIPMENT_GID 											        = SC.SHIPMENT_GID
    AND SC.OTM_SHIPMENT_ACCRUAL_VALUE								>0
 ),
...
MAIN_CTE AS 
 ...
 -- LOCATION_REMARKS REMARK SUB QUERIES        
  ...
  -- ALLOCATION SUB QUERIES     
    , (SELECT ROUND(SUM( A.COST / TO_CURRENCY('EUR' , A.COST_CURRENCY_GID, A.EXCHANGE_RATE_DATE, A.EXCHANGE_RATE_GID)*TO_CURRENCY( 'EUR', SRCTE.OTM_CURRENCY, A.EXCHANGE_RATE_DATE, A.EXCHANGE_RATE_GID)),2) FROM GLOGOWNER.ALLOCATION_ORDER_RELEASE_D A WHERE A.SHIPMENT_GID = S.SHIPMENT_GID AND A.ORDER_RELEASE_GID = ORL.ORDER_RELEASE_GID) APPORTIONED_OTM_SHIPMENT
