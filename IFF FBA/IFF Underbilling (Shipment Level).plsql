--! 05.IFF Underbilling (Shipment Level)
-- > Notes
...
SHIP_COST_SPLIT AS (
      ...
      , SUM(CASE WHEN  SC.INSERT_DATE <= SC.INVOICE_APPROVED_DATE 
                 OR (REPLACE(SC.TRANSPORT_MODE_GID,{DOMAIN}||'.') NOT IN ('OCEAN-FCL','OCEAN-LCL','OCEAN','VESSEL-CO','OCEAN-FCL-ROAD','OCEAN-LCL-ROAD') AND SC.INVOICE_APPROVED_DATE IS NOT NULL)
                THEN SC.SHIPMENT_COST END )                          OTM_SHIPMENT_APPROVED_VALUE
      , SUM(CASE WHEN SC.INSERT_DATE <= SC.INVOICE_REJECTED_DATE 
                 OR (REPLACE(SC.TRANSPORT_MODE_GID,{DOMAIN}||'.') NOT IN ('OCEAN-FCL','OCEAN-LCL','OCEAN','VESSEL-CO','OCEAN-FCL-ROAD','OCEAN-LCL-ROAD') AND SC.INVOICE_REJECTED_DATE IS NOT NULL)
                THEN SC.SHIPMENT_COST END )                          OTM_SHIPMENT_REJECTED_VALUE
      , SUM(CASE WHEN (SC.INSERT_DATE >= SC.INVOICE_APPROVED_DATE OR SC.INVOICE_APPROVED_DATE IS NULL)
                 OR (REPLACE(SC.TRANSPORT_MODE_GID,{DOMAIN}||'.') NOT IN ('OCEAN-FCL','OCEAN-LCL','OCEAN','VESSEL-CO','OCEAN-FCL-ROAD','OCEAN-LCL-ROAD') AND SC.INVOICE_APPROVED_DATE IS NULL)
...
FILTER_CTE AS 
  ( 
    ...
    , ISCTE.INVOICE_GID
    , INVST.UPDATE_DATE
    , (SELECT SUM(INV.ATTRIBUTE11)FROM GLOGOWNER.INVOICE INV WHERE INV.INVOICE_GID= ISCTE.INVOICE_GID AND INV.DOMAIN_NAME = {DOMAIN})                                                               TOTAL_SHIPMENT_INVOICE_NET_AMOUNT
...
(
  SELECT 
    SHIPMENT_GID
    , SUM(CASE WHEN CODE = 'BASE_COST'           THEN SHIPMENT_COST END)  BASE_COST
    , SUM(CASE WHEN CODE = 'FUEL_SURCHARGE_COST' THEN SHIPMENT_COST END)  FUEL_SURCHARGE_COST
    , SUM(CASE WHEN CODE = 'DELAYS_COST'         THEN SHIPMENT_COST END)  DELAYS_COST
    , SUM(CASE WHEN CODE = 'CANCELLATION_COST'   THEN SHIPMENT_COST END)  CANCELLATION_CHARGE
    , SUM(CASE WHEN CODE = 'OTHER_COST'          THEN SHIPMENT_COST END)  MISCELLANEOUS
  FROM 
    (
      SELECT
          SHIPMENT_GID
        , CASE WHEN COST_TYPE = 'B' THEN 'BASE_COST'
...
MAIN_CTE AS 
  ...
  -- SHIPMENT TABLE DATA
    ...
    , TO_CHAR(ROUND(S.TOTAL_WEIGHT, 2),'FM999G999G990D00')|| ' '|| S.TOTAL_WEIGHT_UOM_CODE                                                        GROSS_WEIGHT
  ...
  -- INVOICE TABLE DATA
    ...
    , TO_CHAR(FCTE.UPDATE_DATE, 'DD/MM/YYYY hh24:mi:ss')             INVOICE_STATUS_DATE
  ...
  -- INVOICE ATTRIBUTES *UPDATE*
    , INV.ATTRIBUTE2                                                 OTM_INVOICE_STATUS
    , INV.ATTRIBUTE14                                                INVOICE_CURRENCY
    , DECODE(S.ATTRIBUTE9, 'AUDIT-ONLY', NULL, CASE WHEN SRCTE.FBA_PROCESS_MODE = 'MANUAL' THEN CAST(INV.ATTRIBUTE13 AS VARCHAR(100)) ELSE CAST(IRCTE.CELATON_TOTAL_COST_WITH_VAT AS VARCHAR(100)) END)       OTM_INVOICE_AMOUNT_GROSS
  ...
  -- SUB QUERIES
  -- INVOICE SUB QUERIES
    , CASE WHEN S.ATTRIBUTE9 = 'AUDIT-ONLY' THEN NULL ELSE (SELECT MAX(REPLACE(VAT_CODE_GID, VA.DOMAIN_NAME||'.')) FROM GLOGOWNER.VAT_ANALYSIS  VA WHERE VA.INVOICE_GID = INV.INVOICE_GID) END            TREATMENT_CODE
  ...
  -- LOCATION_REMARKS REMARK SUB QUERIES        
    , (SELECT MAX(LR.REMARK_TEXT) FROM GLOGOWNER.LOCATION_REMARK LR WHERE LR.REMARK_QUAL_GID = LR.DOMAIN_NAME||'.VAT_NUMBER' AND LR.LOCATION_GID = SPL.LOCATION_GID)                                       CARRIER_VAT_NUMBER
  ...
  --CTE TABLES 
    ...
    -- CTE JOINS
      ...
      AND ROUND(SCCTE.APPROVED_INVOICE_NET_AMOUNT,2) - ROUND( S.TOTAL_ACTUAL_COST,2)  <0
...
SELECT        
    ...
    , UNDER_BILLED_AMOUNT                                                    "OTM Shipment Underbilled Amount"
  --, TO_CHAR(OTM_SHIPMENT_REJECTED_VALUE,'FM999G999G999G990D00')            "OTM Shipment Rejected Value"
    ...
    , TO_CHAR(FINAL_COST_OK_DATE, 'DD/MM/YYYY hh24:mi:ss')                   "Cost Ok Date"
