--! 04.IFF Invoice Backup (Shipment Level) | Ocean Freight
-- > Notes
    --   > Only 'COST4', 'COST5' and 'OTHERS' are used from the COST_CATEGORY_CTE and categorirs need to be reviewed
    --   > ORDER_NUMBER column uses refnum qualifier "ORDER_NUMBER" which is not used for FKABI, thus will always show blank.
    ...
    --   > Filter_CTE uses INVOICE_GID, SHIPMENT_GID instead of ORDER_RELEAE_GID
...
INV_SHIP_CTE AS
  ( 
    ...
    DISTINCT
       ...
       -- , OM.TOTAL_WEIGHT_BASE / NULLIF(SUM(TOTAL_WEIGHT_BASE) OVER (PARTITION BY INV.INVOICE_GID),0)                           WEIGHTED_INVOICE
       -- , OM.TOTAL_WEIGHT_BASE / NULLIF(SUM(TOTAL_WEIGHT_BASE) OVER (PARTITION BY OM.SHIPMENT_GID),0)                           WEIGHTED_SHIPMENT
    FROM
...
FILTER_CTE AS 
  ( 
    ...
    , ROUND((SELECT SUM(SHIPMENT_COST) FROM SHIP_COST_CTE SC WHERE SC.SHIPMENT_GID = ISCTE.SHIPMENT_GID ),2)                                                                                        APPORTIONED_OCEAN_FREIGHT                  
...
MAIN_CTE AS 
  ...
  -- SUB QUERIES
    ...
    , RO.CURRENCY_GID                                                                                                                                                                                     MASTER_RATE_CURRENCY
   ...
   -- , FCTE.APPORTIONED_OCEAN_FREIGHT    APPORTIONED_OTM_SHIPMENT_OCEAN
    ...
    , (CC.DELAYS_COST + CC.CANCELLATION_CHARGE + CC.MISCELLANEOUS) SHIPMENT_ACCESSORIAL_COST
    ...
    -- ORDER RELEASE REFNUM SUB QUERIES 
    , (SELECT LISTAGG(ORR.ORDER_RELEASE_REFNUM_VALUE, ',') WITHIN GROUP( ORDER BY 1) FROM GLOGOWNER.ORDER_RELEASE_REFNUM ORR, GLOGOWNER.ORDER_MOVEMENT OM WHERE ORR.ORDER_RELEASE_REFNUM_QUAL_GID = ORR.DOMAIN_NAME||'.BILLING_ENTITY' AND ORR.ORDER_RELEASE_GID = OM.ORDER_RELEASE_GID AND OM.SHIPMENT_GID = S.SHIPMENT_GID)     CUSTOMER_SALES      
      -- IFF Tax Mapping
     , IIBB_CABA_AMOUNT
     , IIBB_BA_AMOUNT
     , CGST_AMOUNT
    ...
    WHERE
     ...
     -- AND SR.SHIPMENT_REFNUM_QUAL_GID                   = S.DOMAIN_NAME ||'.FBA_PROCESS_MODE' 
    ...
    -- CTE JOINS
     ...
     -- AND INV.INVOICE_GID                               = CC.INVOICE_GID 
...
SELECT        
    ...
    , TO_CHAR(SHIPMENT_ACCESSORIAL_COST,'FM999G999G999G990D00')                                                   "OTM Shipment Freight Accessorial Cost"
   ...
   -- , TO_CHAR(ROUND(NVL(SHIPMENT_BASE_COST, 0) + NVL(SHIPMENT_ACCESSORIAL_COST, 0)+NVL(TOTAL_FUEL_SURCHARGE, 0) ,2),'FM999G999G999G990D00')    "OTM Order / Shipment Total Cost"
    ...
    , MASTER_RATE_CURRENCY                                                                                        "Rate Card Master Currency"
