 --! 01.IFF Open Shipment (Shipment Level)
 
  -- > Notes
    --   > Accessorial codes can vary between accounts
    --   > Attribute columns need to be validated for each account
    --   > 4 sub query columns refer to the INVOICE_SHIPMENT table when the query excludes and shipments with a value in INVOICE_SHIPMENT table, thus returning null results (OTM_INVOICE_AMOUNT_NET, TREATMENT_CODE, INVOICE_CURRENCY, OTM_INVOICE_AMOUNT_GROSS)
    --   > Query filters out any shipments whith status "COST OK", however the correct status is "OST_OK" 
    --   > 9 Columns are not mapped to any database fields and are set to 'NULL' values  (CARRIER_INVOICE_NUMBER,CARRIER_INVOICE_DATE,OTM_INVOICE_CURRENCY,OTM_INVOICE_VALUE, CARRIER_INVOICE_RECEIPT_DATE, CARRIER_INVOICE_VALUE_MATCHED, INVOICE_STATUS_DATE, OTM_INVOICE_STATUS)
...
FILTER_CTE AS 
  ( 
    ...
    WHERE  
      ...
      AND NOT EXISTS (SELECT  1 FROM  GLOGOWNER.IE_SHIPMENTSTATUS IES,GLOGOWNER.SS_STATUS_HISTORY SSH WHERE IES.STATUS_CODE_GID = IES.DOMAIN_NAME||'.COST_OK' AND SSH.SHIPMENT_GID = S.SHIPMENT_GID AND IES.I_TRANSACTION_NO = SSH.I_TRANSACTION_NO)
      AND REPLACE(S.TRANSPORT_MODE_GID,{DOMAIN}||'.') NOT IN ('OCEAN-FCL', 'OCEAN-LCL', 'OCEAN', 'VESSEL-CO', 'OCEAN-FCL-ROAD', 'OCEAN-LCL-ROAD')
...
MAIN_CTE AS 
  (
    ...
    -- SHIPMENT ATTRIBUTES *UPDATE*
      --S.ATTRIBUTE9                                                 FBA_RESPONSIBILITY_STATUS
    , S.ATTRIBUTE1                                                   OTM_SHIPMENT_STATUS
    --! COST OK DATE SHOULD always be blank because COST_OK is filtered out 
