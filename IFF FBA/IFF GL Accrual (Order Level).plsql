--! 03.IFF GL Accrual (Order Level)
-- > Notes
    --   > Only 'COST4', 'COST5' and 'OTHERS' are used from the COST_CATEGORY_CTE and categorirs need to be reviewed
    --   > ORDER_NUMBER column uses refnum qualifier "FKABI.ORDER_NUMBER" which is not used for FKABI, thus will always show blank.
...
FILTER_CTE AS 
  (
    ...
    , GLOGOWNER.SS_STATUS_HISTORY                                  SSH
    WHERE 
      S.SHIPMENT_GID                                               = ISCTE.SHIPMENT_GID 
      AND S.DOMAIN_NAME                                            = {DOMAIN}
      AND S.SHIPMENT_GID                                           = SSH.SHIPMENT_GID
      AND IES.STATUS_CODE_GID                                      = S.DOMAIN_NAME||'.COST_OK'
      ...
      AND NOT EXISTS (SELECT 1 FROM GLOGOWNER.INVOICE_SHIPMENT INV_S, GLOGOWNER.INVOICE_STATUS INVST WHERE INV_S.SHIPMENT_GID = S.SHIPMENT_GID AND INV_S.INVOICE_GID = INVST.INVOICE_GID AND INVST.STATUS_VALUE_GID IN (INVST.DOMAIN_NAME||'.APPROVAL_APPROVED_AUTO',INVST.DOMAIN_NAME||'.APPROVAL_APPROVED_MANUAL'))
...
MAIN_CTE AS 
  ...
  -- CTE JOINS
      ...
      AND FCTE.INVOICE_GID                              = INV.INVOICE_GID  (+)
